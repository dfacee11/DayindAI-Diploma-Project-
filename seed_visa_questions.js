...
async function seed() {
  for (const [cityId, questions] of Object.entries(data)) {
    await db.collection('visa_questions').doc(cityId).set(questions);
    console.log(`✅ ${cityId} uploaded`);
  }
  console.log('🎉 Done! All visa questions saved to Firestore.');
  process.exit(0);
}
